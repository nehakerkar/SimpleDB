...
public class Buffer {
   ...
   public static int defaultRefCount; 
   int referenceValue = 0; // 0 means buffer is ready for replacement
   ...
   /**
    * Initializes the buffer's reference count to the value of 
    * @link Buffer.defaultrefcount
    */
   void initRefCount() {
      if (pins == 0)
      {
    	  referenceValue = defaultRefCount;
      }
   }

   /**
    * Decrements the buffer's reference count by 1
    */
   void decrRefCount() {
      referenceValue--;
   }
   ...
   /**
    * Returns true if the buffer has referenceCount 
   ...
   /**
    * Append the contents of page to the backup file
    */
   public int saveBlock()
   {
 	  Block savedblk=contents.append("Savedblocks.txt");
 	  return savedblk.number();
   }
 
   /**
    * Added to retrieve blocks from the file
    */
   public void restoreBlock(int blkoffset)
   {
	   Block retrievedblk=new Block("Savedblocks.txt",blkoffset);
	   contents.read(retrievedblk);
   }
}