...
public class RecoveryMgr {
    ...
    */
   public int setInt(Buffer buff, int offset, int newval, int bkpfileoffset) {
      int oldval = buff.getInt(offset);
      Block blk = buff.block();
      if (isTempBlock(blk))
         return -1;
      else
         return new UpdateRecord(txnum, blk.fileName(), blk.number(), bkpfileoffset).writeToLog();
   }
    ...
    * @param newval the value to be written
    */
   public int setString(Buffer buff, int offset, String newval, int bkpfileoffset) {
